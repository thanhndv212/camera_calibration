"""Unit tests for the shared calibration helpers."""
...
from camera_calibration._shared import (
...
class ParseResolutionTests(unittest.TestCase):
    def test_parse_x_separator(self) -> None:
    ...
    def test_parse_uppercase_x(self) -> None:
    ...
    def test_parse_none_returns_default(self) -> None:
        from camera_calibration._shared import DEFAULT_RESOLUTION
        self.assertEqual(parse_resolution(None), DEFAULT_RESOLUTION)
...
class ResolveOutputPrefixTests(unittest.TestCase):
    def test_prefix_created(self) -> None:
        ...
        self.assertIn("test_camera", prefix.name)
        self.assertIn("session", prefix.name)
